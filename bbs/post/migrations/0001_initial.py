# Generated by Django 3.2.13 on 2022-04-16 10:22

from django.conf import settings
from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Topic',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('createtime', models.DateTimeField(auto_now_add=True, help_text='创建时间')),
                ('last_modified', models.DateTimeField(auto_now=True, help_text='更新时间')),
                ('title', models.CharField(help_text='话题标题', max_length=255, unique=True)),
                ('content', models.TextField(help_text='话题内容')),
                ('is_online', models.BooleanField(default=True, help_text='话题是否在线')),
                ('user', models.ForeignKey(help_text='关联用户表', on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'ordering': ['-createtime'],
                'abstract': False,
            },
        ),
        migrations.CreateModel(
            name='Comment',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('createtime', models.DateTimeField(auto_now_add=True, help_text='创建时间')),
                ('last_modified', models.DateTimeField(auto_now=True, help_text='更新时间')),
                ('content', models.TextField(help_text='评论内容')),
                ('up', models.IntegerField(default=0, help_text='支持')),
                ('down', models.IntegerField(default=0, help_text='反对')),
                ('topic', models.ForeignKey(help_text='关联话题', on_delete=django.db.models.deletion.CASCADE, to='post.topic')),
            ],
            options={
                'ordering': ['-createtime'],
                'abstract': False,
            },
        ),
    ]
